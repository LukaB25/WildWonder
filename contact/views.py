...
def contact(request):
    """
    A view to return the contact page from :model:
    `contact.ContactAboutSection` and :model:`contact.ContactFormMessage`.

    **Context**
    ``contact``
        An instance of :model:`contact.ContactAboutSection`
        to display the contact information.

    ``contact_form``
        An instance of :model:`contact.ContactFormMessage`
